...
- `BottomStatusBar` 保留类名和两行状态格式，但 Windows inline 路径不再创建独立 JLine `Status` / scroll region：两行状态作为多行 prompt 的前缀，与最终 `* ` 输入行一起交给 LineReader 的同一个 Display 重排。不要重新引入 `cursor_address`、`Status` border 或第二套 resize handler，否则 Windows Terminal 最小化/恢复会把旧状态区卷入主屏并累积残影。状态提示保留两类信息：上层模式 + MCP/Skill 摘要，下层 Auto Model / model / phase / ctx 百分比与 token / cost / elapsed / cwd。关键字段可用克制的 JLine `AttributedString` 彩色样式突出，但纯文本格式和宽度裁剪逻辑要保持稳定。`ctx` 表示当前仍会带入下一轮请求的上下文估算；`in/out/cache` 表示最近任务的 LLM 调用统计，二者不要混用。
...
- `LineReader` 使用 `PaiCliCompleter` 做上下文补全：`/model` provider、`/mcp` 子命令与 server、`/skill` 子命令与 skill name、`/task` / `/browser` / `/snapshot` 子命令、`@image:` 本地路径、本地 `@path` 和 MCP resource `@server:uri` 引用都应从同一个 completer 出口维护。`@image:file://` 在 Windows 需兼容标准 `file:///C:/...` 与常见的 `file://C:/...`、`file://C:\...`，并支持空格、中文和 `%20`。
