 ...
 * JLine 输入模型托管的两行状态提示。
 *
 * <p>状态内容作为多行 prompt 的前两行交给 LineReader 统一重排，不再建立独立
 * scroll region。这样 Windows Terminal 在窗口还原/最大化时只维护一个显示模型，
 * 不会把旧状态区卷入主屏形成残影。
 *
...
public final class BottomStatusBar implements AutoCloseable {
    ...
    String inputPrompt() {
        ...
        // 预留最后一列，避免完整列宽触发 Windows Terminal 的自动换行位。
        int cols = Math.max(20, TerminalCapabilities.safeSize(terminal).getColumns() - 1);
        List<AttributedString> lines = formatStatusLines(info, cols);
        String prompt = lines.get(0).toAnsi(terminal)
                + "\n"
                + lines.get(1).toAnsi(terminal)
                + "\n* ";
        // LineReader prompt pattern 用 % 表示格式指令；状态里的 1% 必须写成 1%%。
