...
public final class InlineRenderer implements Renderer {
    ...
    public String inputPrompt() {
        return statusBar == null ? "* " : statusBar.inputPrompt();
    }
    ...
    private int acceptedInputRows(String input) {
        ...
        String prompt = stripAnsi(inputPrompt()).replace("%%", "%")
                .replace("\r\n", "\n").replace('\r', '\n');
        String[] promptParts = prompt.split("\n", -1);
        String[] parts = text.split("\\R", -1);
        int rows = 0;
        for (int i = 0; i < promptParts.length - 1; i++) {
            rows += Math.max(1, (displayWidth(promptParts[i]) + cols - 1) / cols);
        }
        int finalPromptWidth = displayWidth(promptParts[promptParts.length - 1]);
        for (int i = 0; i < parts.length; i++) {
            int cells = displayWidth(parts[i]) + (i == 0 ? finalPromptWidth : 0);
